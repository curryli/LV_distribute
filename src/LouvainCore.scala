...
object LouvainCore {
  ...
  def louvain(sc:SparkContext, graph:Graph[VertexState,Long], minProgress:Int=1,progressCounter:Int=1) : (Double,Graph[VertexState,Long],Int)= {
    ...
    do { 
       ...
       oldMsgs.unpersist(blocking=false)           //手工的删除一个 RDD而不是等他自动从缓存中清除，使用RDD.unpersist()方法。
       updatedVerts.unpersist(blocking=false)
       prevG.unpersistVertices(blocking=false)  //顶点释放
	   ...
	   if (!even) {
	     println("  #  : "+java.text.NumberFormat.getInstance().format(updated))
